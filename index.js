...
client.on("message", async message => {
  ...
  if(command === "güvenli") {
        ...
        message.channel.send(embed.addField("Güvenli Liste <a:kaytk:841782363687092224>", safemember.length > 0 ? safemember.map(g => (message.guild.roles.cache.has(g.slice(1)) || message.guild.members.cache.has(g.slice(1))) ? (message.guild.roles.cache.get(g.slice(1)) || message.guild.members.cache.get(g.slice(1))) : g).join('\n') : "Herhangi bir üye bulunamadı.<a:kaytcarp:841782363218509835>"));
  ...
  if(command === "ytkapat") {
    ...
    message.channel.send(new MessageEmbed().setColor("#00ffdd").setTitle('İzinler Kapatıldı <a:kaytk:841782363687092224>').setDescription(`Rollerin yetkileri kapatıldı <a:kaytk:841782363687092224>`).setTimestamp()).catch(); 
  };
})
...
function punish(strg, hammer) {
  let member = client.guilds.cache.get(config.guildID).members.cache.get(strg);
  if (!member) return;
  if (hammer == "jail") return member.roles.cache.has(config.booster) ? member.roles.set([config.booster, config.jail]) : member.roles.set([config.jail]).catch(err => console.log('Kişiyi Sunucudan atamadım. Lütfen Kontrol ediniz.<a:kaytcarp:841782363218509835>'));
  if (hammer == "ban") return member.ban({ reason: "Guard Tarafından Banlandı. <a:kaytk:841782363687092224>" }).catch(err => console.log('Kişiyi banlayamadım. Lütfen kontrol ediniz.<a:kaytcarp:841782363218509835>'));
};
...
client.on('guildMemberAdd', async member => {
  ...
  client.channels.cache.get(config.Log.BotAddLog).send(new MessageEmbed().setTitle('**Bir Bot Eklendi <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından bot eklendi. \n ${raviwen.executor} ve ${member} banladım.`).addField('Eklenen Bot', member))
})
...
client.on("guildMemberRemove", async member => {
  ...
  if ( !raviwen || !raviwen.executor || güvenli(raviwen.executor.id)) return;
  punish(raviwen.executor.id, "ban")
  client.channels.cache.get(config.Log.KickLog).send(new MessageEmbed().setTitle('**Üye Tarafından Kick Atıldı <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından kick atıldı.`).addField('Kicklenen Üye', member))
})
...
client.on('guildBanAdd', async (guild, user) => {
  ...
  if (!raviwen || !raviwen.executor || güvenli(raviwen.executor.id)) return;
  punish(raviwen.executor.id, "ban");
  guild.members.unban(user.id, "Sağ Tık ile banlandığını için banını geri açtım.").catch(err => console.log('Kişinin banını açamadım.<a:kaytcarp:841782363218509835>'))
  client.channels.cache.get(config.Log.BanLog).send(new MessageEmbed().setTitle('**Üye Tarafından Ban Atıldı <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından ban atıldı. \n Ban atan kişiyi banladım ve banlanan kişinin banını kaldırdım.<a:kaytk:841782363687092224>`).addField('Banlanan Üye', user))
...
client.on('roleCreate', async role => {
  ...
  client.channels.cache.get(config.Log.RoleLog).send(new MessageEmbed().setTitle('**Bir Rol Oluşturuldu <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından rol oluşturuldu \n Rolü oluşturan kişiyi Jail Attım ve Oluşturulan rolü sildim.<a:kaytk:841782363687092224>`))
})
...
client.on('channelCreate', async channel => {
  ...
  punish(raviwen.executor.id, "jail")
  client.channels.cache.get(config.Log.ChannelLog).send(new MessageEmbed().setTitle('**Bir Kanal Oluşturuldu <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından Kanal oluşturuldu \n Kanalı oluşturan kişiyi Jail Attım ve Oluşturulan kanalı sildim.<a:kaytk:841782363687092224>`))
})
...
client.on("channelDelete", async channel => {
  ...
  client.channels.cache.get(config.Log.ChannelLog).send(new MessageEmbed().setTitle('**Bir Kanal Silindi <a:kaytcarp:841782363218509835>**').setColor("RED").setTimestamp().setDescription(`${raviwen.executor} \`${raviwen.executor.id}\` Tarafından kanal silindi. \n Kanalı silen kişiyi banladım ve kanalı yeniden oluşturdum. (Deleted Yazıyorsa Denetim Kaydını İnceleyiniz) \n Geri oluşturulan kanalın ayarlarını düzenleyiniz.<a:kaytk:841782363687092224>`).addField('Silinen Kanal', channel))
